...
export default function ProfileRequestsPage() {
    ...
    if (loading && requests.length === 0) {
        return <div className="p-20 text-center font-bold text-gradient-gold">Loading Requests...</div>;
    }
                ...
                {requests.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-4 md:gap-6">
                        {requests.map((request) => (
                            <div key={request._id} className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-500">
                                <div 
                                    onClick={() => toggleExpand(request._id)}
                                    className="px-6 sm:px-8 py-5 bg-gray-50/50 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 cursor-pointer hover:bg-gray-100/50 transition-colors"
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`transition-transform duration-300 ${expandedIds.has(request._id) ? 'rotate-180' : ''}`}>
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4 text-[#D4AF37]">
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                                            </svg>
                                        </div>
                                        <div className="w-12 h-12 rounded-full bg-gradient-gold flex items-center justify-center text-black font-black text-lg shadow-lg border-2 border-white flex-shrink-0">
                                    ...
                                    <div className="flex gap-3">
                                        <button
                                            ...
                                            className="px-6 py-2.5 bg-gradient-gold text-black font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg hover:brightness-110 active:scale-95 transition-all"
                                        >
                                            Approve
                                        </button>
                                        <button
                                            onClick={(e) => handleReject(e, request._id)}
                                            className="px-6 py-2.5 bg-red-50 text-red-600 border border-red-100 font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-red-100 active:scale-95 transition-all"
                                        >
                                            Reject
                                        </button>
                                    </div>
                                    ...
                                    <div className="p-6 sm:p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 bg-white animate__animated animate__fadeIn">
                                        {[
                                            ...
                                            <div key={idx} className={field.full ? 'md:col-span-2 lg:col-span-4' : ''}>
                                                <label className="block text-[9px] font-black text-[#A67C00] uppercase tracking-widest mb-1.5">{field.label}</label>
